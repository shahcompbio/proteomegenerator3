# kentsislab/proteomegenerator3: Usage
...
The samplesheet uses a long format where each row represents a single file associated with a sample. Multiple samples from the same patient/subject can share a `subject_id`.
...
```bash
nextflow run kentsislab/proteomegenerator3 --input ./samplesheet.csv --outdir ./results --genome GRCh37 -profile docker
```
...
```bash
nextflow run kentsislab/proteomegenerator3 -profile docker -params-file params.yaml
```
