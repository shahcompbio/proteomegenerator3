process SEQKIT_RMDUP {
    ...
    tuple val(meta), path("${prefix}.dup_seq_ids.txt"), emit: dup_seq_ids
    tuple val(meta), path("${prefix}.dup_seqs.${extension}"), emit: dup_seqs
    ...
    seqkit \\
        rmdup \\
        --threads ${task.cpus} \\
        -D ${prefix}.dup_seq_ids.txt \\
        -d ${prefix}.dup_seqs.${extension} \\
        ${args} \\
        ${fastx} \\
        -o ${prefix}.${extension} \\
        2>| >(tee ${prefix}.log >&2)
    ...
    touch ${prefix}.dup_seq_ids.txt
    touch ${prefix}.dup_seqs.${extension}
    echo \\
