...
- `reannotate_gtf` now assigns one `NovelGene<N>` per novel **locus** instead of one per novel **transcript**. The rename loop iterates over `groupby("transcript_id")` (Go: `groupOrder`) and minted the novel gene ID from the same counter used for the transcript ID, so a distinct gene was created for every transcript and no two isoforms of a novel locus could ever share a `gene_id` — by construction, not as an artifact of the data. The assembler's own locus (`gene_id` on the input, passed through by `gffcompare`, e.g. `g:chr16:+:comp-189`) was read into `old_gene_id` and written to the id-mapping TSV, but never used to assign the new ID. Novel gene IDs are now keyed on that input locus. On the union transcriptome of a 99-sample cohort this had split 453,361 novel transcripts across 234,859 real loci into 453,361 single-transcript pseudo-genes, inflating the novel gene count ~1.9x (worst locus: 436 isoforms reported as 436 genes) and making any gene-level analysis of novel loci — including differential transcript _usage_, which is undefined for a single-isoform gene — impossible. Transcript IDs and reference (`ENSG`/`ENST`) assignments are unchanged. Fixed identically in `bin/reannotate_gtf.go` (what the container builds and the pipeline runs) and `bin/reannotate_gtf.py`
